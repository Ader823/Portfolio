...
// game variables
let startScene;
let gameScene, player, scoreLabel, shootSound, hitSound, jumpSound, block, wall;
let gameOverScene;
...
function GroundSpawning(dt) {
    ...
    if (ground[ground.length - 1].x + blockSize < app.view.width) {
        // How long the chain
        let chain = Math.floor(getRandom(2, 20));
...
function spawnBlocks(chainLength, gap) {
    ...
    for (let i = 0; i < chainLength; i++) {
        ...
        block = new Block(app.view.width + i * blockSize + gap * blockSize, app.view.height - groundHeight, blockSize, blockSize);
        ground.push(block);
        gameScene.addChild(block);

        // If there is an obstacle
        if (i == obstacleIndex) {
            // 75% chance to be obstacle, 25% to be wall to destroy
            if (Math.random() <= 0.75) {
                // 50% chance to be jumping obstacle, 50% chance to be ducking
                if (Math.random() <= 0.5) {
                    // At that index, spawn a block on top of the ground block
                    wall = new Wall(app.view.width + i * blockSize + gap * blockSize, app.view.height - groundHeight - blockSize * 1, blockSize, blockSize * 1, "images/block.png");
                    walls.push(wall);
                    ...
                    if (Math.random() <= 0.5) {
                        ...
                        wall = new Wall(app.view.width + i * blockSize + gap * blockSize, app.view.height - groundHeight - blockSize * (9 + gapSize), blockSize, blockSize * 6, "images/block.png");
                        walls.push(wall);
                        gameScene.addChild(wall);
                    }
                } else {
                    wall = new Wall(app.view.width + i * blockSize + gap * blockSize, app.view.height - groundHeight - blockSize * (11.3), blockSize, blockSize * 10, "images/block.png");
                    walls.push(wall);
                    gameScene.addChild(wall);
                }

            } else {
                wall = new Wall(app.view.width + i * blockSize + gap * blockSize, app.view.height - groundHeight - blockSize * 10, blockSize, blockSize * 10, "images/block.png");
